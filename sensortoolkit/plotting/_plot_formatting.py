...
def met_scatter_lims(met_data, param, met_param, xlims, ylims, serials,
                     depvar_df_list, dep_var):
    """Set axes limits for plots generated by ``met_influence()``.
    ...
    Args:
        ...
        dep_var (str):
            The dependent variable (y-axis) that will be displayed on the
            resulting figure. Either 'normalized', 'diff', or 'absdiff'.
    ...
    for i, df in enumerate(depvar_df_list, 1):
        avg_df[f'Sensor_{i}_{dep_var.title()}_{param}_Value'] = df[f'{dep_var.title()}_{param}_Value']

    avg_df[f'mean_{dep_var.title()}_{param}_Value'] = avg_df.mean(axis=1)
    ...
    if ylims is None:
        ymin = avg_df[f'mean_{dep_var.title()}_{param}_Value'].quantile(0.01)
        ymax = avg_df[f'mean_{dep_var.title()}_{param}_Value'].quantile(0.99)
