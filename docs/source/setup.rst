...
`sensortoolkit` includes various setup modules for assisting users in organizing sensor
and reference data as well as outputs generated by the library such as figures,
evaluation datasets, and reports into several sub-directories in the user's
working directory.
...
Also, create a variable ``work_path`` (or something similar) that will indicate the full directory path for the
parent directory of the script. This will also be the parent directory for data, figures, and reports, so users
may wish to create a new or empty folder for evaluations.
