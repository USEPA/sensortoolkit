...
the `evaluation.py` example, mention of ``sensor_object`` below refers to the ``AirSensor``
object instantiated under the `Testing Attribute Objects - The AirSensor Object` section.

Accessing Sensor Data
^^^^^^^^^^^^^^^^^^^^^
Sensor datasets are access via the ``sensor_object.data`` attribute, which stores dataframes
...
.. code-block:: python
  ...
  {'SN01': # Key may differ depending on choice of serial identifiers
   ...
   'SN02': # Key may differ depending on choice of serial identifiers
       {'1-minute': pandas DataFrame object, # Key may differ depending on sensor sampling freq.
        '1-hour': pandas DataFrame object,
        '24-hour': pandas DataFrame object},
   'SN03': # Key may differ depending on choice of serial identifiers
       {'1-minute': pandas DataFrame object, # Key may differ depending on sensor sampling freq.
        '1-hour': pandas DataFrame object,
        '24-hour': pandas DataFrame object}
  }
...
.. note::
  ...
  choice for serial identifiers (e.g., ``SN01``). Additionally, the first key within
