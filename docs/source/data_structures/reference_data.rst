...
the `evaluation.py` example, mention of ``reference_object`` below refers to the ``ReferenceMonitor``
...
Reference datasets are access via the ``reference_object.data`` attribute, which stores dataframes
in a dictionary structure at both 1-hour and 24-hour averaged intervals for each
